...
const struct pci_device_id id_table[] = {
    { PCI_DEVICE_CLASS(PCI_CLASS_STORAGE_SATA_AHCI, PCI_ANY_ID) },
    {0,}
};
...
{
    ahci_driver_data_t *pDrvData = NULL;
    uint32_t start, len;
    (void)(pId);
    ...
    pDrvData = kzalloc(sizeof(ahci_driver_data_t), GFP_KERNEL);
    ...
    if (pci_request_region(pPciDev, AHCI_PCI_BAR, KBUILD_MODNAME) != 0) {
        printk(KERN_ERR "%s: Error at pci_request_region()!\n", KBUILD_MODNAME);
        goto ERR1;
    }

    start = pci_resource_start(pPciDev, AHCI_PCI_BAR);
    len = pci_resource_len(pPciDev, AHCI_PCI_BAR);

    if ((len == 0) || (((pci_resource_flags(pPciDev, AHCI_PCI_BAR) & IORESOURCE_MEM) != IORESOURCE_MEM))) {
        printk(KERN_ERR "%s: Unsupported hardware!\n", KBUILD_MODNAME);
        goto ERR2;
    }

    if (pDrvData->debug)
        printk(KERN_INFO "%s: PCI BAR[%d]: offset 0x%08x, length 0x%08x\n", KBUILD_MODNAME, AHCI_PCI_BAR, start, len);

    pDrvData->pAhciMem = pci_iomap(pPciDev, AHCI_PCI_BAR, len);

    ahci_controller_enable(pDrvData);
    ...
    // ERROR!!!
ERR2:
    if (pDrvData->pAhciMem)
        pci_iounmap(pPciDev, (void *)pDrvData->pAhciMem);

    pci_release_region(pPciDev, AHCI_PCI_BAR);
...
static void device_remove(struct pci_dev *pPciDev)
{
    ...
    ahci_controller_disable(pDrvData);
